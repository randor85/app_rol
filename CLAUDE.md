...
Lo único que sigue siendo distinto, a propósito y de forma disclosed: **el sistema de tiradas de dados.** Dane usa `rollAttack(name, modifier)` / `rollGenericDamage(...)` / `displayRollResult(name, result, calc)` con un widget inline de Normal/Advantage/Disadvantage, mientras el resto de la flota usa `rollCheckSave(dado, mod, etiqueta, tipo)` con un panel flotante de dados y `displayRollResult(total, label, breakdown)` (nótese el orden de argumentos invertido entre ambos). Unificarlo implicaría reescribir decenas de `onclick` con modificadores de combate reales — se dejó fuera por el riesgo, no por descuido; es un pedido aparte si el usuario lo quiere.

No tiene botón de "Descanso Largo" (gap conocido, nunca pedido, no forma parte de la unificación de arquitectura).
